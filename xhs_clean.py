...
def step1(text):
    ...
    cleaned_content = demojize(text)

    # text_normalization
    cleaned_content = cleaner.text_normalization(cleaned_content)
...
def make_clean(items):
    ...
    with open(fpath,"r",encoding="utf-8") as f:
        ...
        for line_no,line in tqdm(enumerate(datas),total=len(datas)):
            ...
            count = 0
            for key in KEYS:count += val_pro(js_dict[key])
            if count <= VAL_FILETR:
                #return "step1", 0, text['content']
                clean_status = "VAL_FILETR"
                clean_status = 0
