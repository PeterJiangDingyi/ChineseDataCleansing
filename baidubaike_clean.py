...
from special_policy import SpecialPolicies
...
from check_black_words import CheckBlackWords
...
def step1(text):
    ...
    if is_chatperts_text:
        return "cn-baidubaike_IsChatperText",0,cleaned_content

    cleaned_content = SpecialPolicies.RemoveSpamFromContent(cleaned_content,spam=r"(\ncopppy zi B1D2B1K2)")
...
def step2(text):
    ...
    is_sentive = spamchecker.is_spam_text(text)
    if is_sentive:
        return "has_sensitive_words",0,text
    return "",1,text
...
def controller(args):
    ...
    for line_no,input_file in tqdm(enumerate(files),total=len(files)):
        ...
        if not input_file.endswith(".jsonl"): continue
...
def make_clean(items):
    ...
    with open(fpath,"r",encoding="utf-8") as f:
        ...
        for line_no,line in tqdm(enumerate(datas),total=len(datas)):
            ...
            title = js_dict["name"]
            abstract = js_dict["abstract"]
            basic_info = js_dict["basic_info"]
            content = js_dict["content"]
            if title is None or abstract is None or content is None: continue

            '''
            File "/root/anaconda3/envs/LLM/lib/python3.8/site-packages/tomark/tomark.py", line 10, in table
            ...
            if markdown is None or len(markdown) < 10:
                text = "#"+title+"\n"+abstract+"\n"+content
            else:
                text = "#"+title+"\n"+abstract+"\n"+markdown+"\n"+content
            ...
            js_dict["id"] = line_no
            js_dict["source"] = "cn-baidubaike"
            js_dict["subset"] = "baike"
            ...
            del js_dict["name"]
            del js_dict["abstract"]
            del js_dict["basic_info"]
